...
class App extends Component {
  ...
  renderChart() {
    ...
    const parent = d3.select(".svg_parent")
      .attr("width", 1000)
      .attr("height", 150);

    const x = (i) => {
      const prevX = (i > 0) ? (x(i - 1) + (data[i - 1][0].length * (fontScale(i - 1) / 2)) + 20) : 0; 
      return prevX;
    };

    const text = parent.selectAll("text")
      .data(data, d => d[0]); 
    ...
    text.enter()
      ...
      .transition()
      .duration(3000)
      .attr("font-size", (d, i) => fontScale(i)) 
      .attr("opacity", 1);
